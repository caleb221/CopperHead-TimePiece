...
const byte addrPins[2] = {2, 15}; // Segment address pins: A0, A1
...
#define TIME_TO_SLEEP 20//Seconds, update on the minute
...
void updateFromJSON()
{
 ...
 int jYear=jsonTime["yr"];
 int jMonth= jsonTime["mnt"];
 int jDay= jsonTime["dy"];
 int jHour=jsonTime["hr"];
 int jMinute=jsonTime["minute"];
 int jSecond=jsonTime["sec"];
  
  struct tm newTimeinfo;
  newTimeinfo.tm_year=jYear;
  newTimeinfo.tm_mon=jMonth;
  newTimeinfo.tm_mday=jDay;
  newTimeinfo.tm_hour=jHour;
  newTimeinfo.tm_min=jMinute;
  newTimeinfo.tm_sec=jSecond;
  time_t tjson = mktime(&newTimeinfo); 
  //struct timeval updatr ={.tv_sec = tjson};

  DateTime rtcCurrent = rtc.now();
  //make sure JSON is more current before sending it to RTC
if( (rtcCurrent.unixtime() / 86400L )< tjson)
    rtc.adjust(DateTime(jYear,jMonth,jDay,jHour,jMinute,jSecond));
else
  updateJsonTime(rtcCurrent.year(),rtcCurrent.month(),rtcCurrent.day(),rtcCurrent.hour(),rtcCurrent.minute(),rtcCurrent.second());
...
void espSleep()
{
  ...
  u8g2.sendBuffer();
  //SLEEP FINALLY!            
  //esp_sleep_enable_ext1_wakeup(BTN_BTMASK,ESP_EXT1_WAKEUP_ANY_HIGH);
...
// RTC SETUP
  ...
  if(!rtc.begin())
    {
      Serial.println("RTC NOT FOUND X(");  
    }
  if(!rtc.isrunning())
  {
    //POWER WAS LOST X(
    // UPDATE THE RTC FROM JSON FILE IN SPIFFS
    // (SAVED FROM LAST NTP UPDATE)
     updateFromJSON();
  }
//=====================
...
while (!hasSelection )
{
  ...
  switch(dir)
  {
    ...
    if(isWebServing)
    {
        ...
        u8g2.drawStr(5,15,"PW: a123456789");
...
int rtHour = rtcNow.hour();
int rtMin = rtcNow.minute();
Serial.print("RTC hour: ");
Serial.println(rtcNow.hour() );
...
//      hour=hour-1;
 ...
 String hourStr=String(abs(rtHour));
 String minuteStr = String(rtMin);
 ...
 //for(int i=0;i<4;i++)
 //{
  //dopeScreen.setCursor(i);
  dopeScreen.print(timeNow); 
  //Serial.print(timeNow.charAt(i)); 
  delay(150);
 //}
...
// curl -i  -X GET http://192.168.4.1/getTime   
        ...
        updateJsonTime(yr,mnt,dy,hr,mint,sec);
...
{
  isWebServing = true;
  WiFi.softAP("CopperHead TimePiece","a123456789");
  //192.168.4.1
